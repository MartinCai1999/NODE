...
po = pd.read_csv("Canada_Covid.csv")
po = po.iloc[40:85, :]
l = len(po)
df = po
...
def NODE(y2_0):
    ...
    for epoch in range(n_epochs1):
        ...
        y_pred = odeint(model3, y2_0, t_t, method='euler')       
...
# metric = R2Score()
# metric.update(yFinal[3:] * n, y[3:].reshape(-1).cpu().detach())
# print(metric.compute())

# from scipy import stats
# res = yFinal[3:] * n - y[3:].reshape(-1).cpu().detach()
# v = torch.var(res)
# SSR = torch.sum(torch.square(res))
# s = torch.tensor(l - 2)
# lnLi = - s / 2 * torch.log(2 * torch.pi * v) - 1 / (2 * v) * SSR
# BIC = 9 * torch.log(s) - 2 * lnLi
# print(BIC)

plt.plot(t[3:], yFinal[3:] * n, 'b', label='Neural ODE (SEI)')
plt.scatter(t[3:], y[3:].cpu().detach().numpy(), color = 'r', label='Real BA.2 in Canada')
plt.legend(loc='best', prop=font1)
plt.xlabel('$t$', font)
plt.ylabel('$N_I$', font, rotation=0)
xData = [3, 8, 16, 24, 32, 40]
