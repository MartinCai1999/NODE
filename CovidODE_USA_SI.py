...
# metric = R2Score()
# metric.update(yFinal[1:] * n, y[1:].reshape(-1).cpu().detach())
...
plt.plot(t[1:], yFinal[1:] * n, 'b', label='Neural ODE (SI)')
plt.scatter(t[1:], y[1:].cpu().detach().numpy(), color = 'r', label='Real BA.2 in USA')
plt.legend(loc='best', prop=font1)
plt.xlabel('$t$', font)
plt.ylabel('$N_I$', font, rotation=0)
xData = [1, 8, 16, 24, 32, 40]
